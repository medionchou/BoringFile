...
public class Environment {
    ...
    private double[] printConsoleResult() {
        ...
        System.out.println("Served terminal num: " + IntStream.of(served_term).sum());
        System.out.println("Average UAV-Terminal distance: " + DoubleStream.of(st_avd).sum());
        System.out.println("Total steps: " + step);
        System.out.println("UAV average distance: " + avd / (uav.length * (uav.length - 1) / 2));

        double[] a = {avs / terminal_num, 
                      IntStream.of(served_term).sum(), 
                      step,
                      avd / (uav.length * (uav.length - 1) / 2), 
                      DoubleStream.of(st_avd).sum() / IntStream.of(served_term).sum()};
        return a;
    }
    ...
    public static void oneTimeSimulation() throws Exception {
        String termConfig = "cluster_with_grid60_term360/cluster_grid60_term360_std7.0.txt";
    ...
    public static void repetitiveSimulation() throws Exception {
        ...
        for (int l = 0; l < TIMES; l++) {
            for (int j = 0; j < 2; j++) {
                ...
                for (int i = 1; i <= 10; i += 1) {
                    ...
                    String filename = "cluster_with_grid60_term360/cluster_grid" + 60 + "_term" + 360 + "_std" + Double.toString(i) + ".txt";
                    filename = "regular_with_grid60/regular" + i + ".txt";
                    if (j == 0) e = new Environment(filename, uavConfig, type);
                    else e = new Environment(filename, uavConfig, type);
                    UAV.ID = 0;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = 0; j < mean[0].length; j++) {
                ...
                System.out.printf("AVD: %.3f ", mean[i][j][3] / TIMES);
                System.out.printf("UAV-Terminal average distance: %.3f \n", mean[i][j][4] / TIMES);
            }
        }
    }
...
// generateClusterDataFromPython("cluster_grid60_term360_std" + i +
// ".txt"
...
//            d += Point.distance(x[j], y[j], 0, x[k], x[k], 0);
//        }
//    }
//    System.out.println(fn + " " + (d / (num * (num-1) / 2)));
//
//}
