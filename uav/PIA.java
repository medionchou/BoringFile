...
public class PIA extends UAV {
    ...
    public PIA(double x, double y, double z, boolean isOpen) {
        super(x, y, z, isOpen);
    ...
    private Point bestStrategy() {
        ...
        HashMap<Terminal, Double> served_cost = new HashMap<>();
        ...
        for (Terminal t : potentialTerms.keySet()) {
            if (t.withinRange(this)) {
                ...
                if (sir != 0.0d) {
                    owned.add(t);
                    served_cost.put(t, Double.MAX_VALUE);
                    hasTerm = true;
                }
                else nonowned.add(t);
        ...
        for (Strategy st : Strategy.values()) {
            ...
            each_payoff = payoff(owned, served_cost, tmp);
    ...
    private double payoff(HashSet<Terminal> termSet, HashMap<Terminal, Double> served_cost, Point pt) {
        ...
        for (Terminal t : termSet) {
        	double tmp = t.distance(this, pt);
            distance = served_cost.get(t);
        ...
        if (!isImproved) return -1;       
        else {
            for (Terminal t : termSet) {
            	double tmp = t.distance(this, pt);
                served_cost.put(t, tmp);
                td += tmp;
            }
            return 1 / td;
        }
